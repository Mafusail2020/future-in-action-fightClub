...
import { useCities, useCityCases } from '../../api/queries'
...
export function CityPanel() {
  ...
  if (!selectedCityId || !city) return null

  return (
    <aside
      aria-label={`Розв'язані проблеми: ${city.name}`}
    ...
    >
      <header className="flex items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div>
          <p className="font-mono text-[11px] tracking-[0.14em] text-faint uppercase">
            {city.country}
            {city.population ? ` · ${Math.round(city.population / 1000)} тис.` : ''}
          </p>
          <h2
            ref={headingRef}
            tabIndex={-1}
      ...
      <div className="panel-scroll min-h-0 flex-1 overflow-y-auto">
        {cases.isPending && <Spinner label="Завантажуємо кейси…" />}
        {cases.isError && (
          <ErrorBox message="Не вдалося завантажити кейси." onRetry={() => cases.refetch()} />
        )}
        {cases.data?.length === 0 && (
          <EmptyState
            title="Для цього міста ще немає кейсів"
            hint="База рішень поповнюється — загляньте пізніше"
          />
        )}
        {cases.data && cases.data.length > 0 && <CaseList cases={cases.data} />}
      </div>
    </aside>
  )
}
